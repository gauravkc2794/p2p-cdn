 ...
 */
class UdpCDNServerHelper
{
public:
  /**
   * Create UdpCDNServerHelper which will make life easier for people trying
   * to set up simulations with echos.
   *
   * \param port The port the server will wait on for incoming packets
   */
  UdpCDNServerHelper (uint16_t port);
...
public:
   ...
   */
  UdpPeerClientHelper (Address ip, uint16_t port);
  /**
   * Create UdpEchoClientHelper which will make life easier for people trying
   * to set up simulations with echos. Use this variant with addresses that do
   ...
   */
  UdpPeerClientHelper (Address addr);
